...
#include <sdl.h>
#include <SDL_Image.h>
#include <vector> // vector is an array with size 
...
float deltaTime = 1.0f / FPS; // time passing btwn the frame in seconds
...
float  playerMoveSpeedPx = 65.0f; // pixels per second 
...
float playerFireRepeatDelay = 1.0f; // in seconds
float bulletspeedPx = 500;//px/sec
...
void update()
{
	// called every frame at FPS

	if (isUpPressed)
	{
		Player.Dst.y -= playerMoveSpeedPx * deltaTime;
	...
	if (isShootPressed && playerShootCooldownTimer <= 0.0f)
	{
		std::cout << "shoot\n";
		// creating new bullet 
		sprite projectiles = sprite(pRenderer, "...Assets/bullet.png");
		...
		// adding bullet to container ( to the end of the array)
	...
	{
		// to acess elements of an array we use the array acess operators[]

		// get a reference to the bullet in the containers
		sprite* someprojectiles = &bulletcontainer[i];
		someprojectiles->Dst.x += bulletspeedPx * deltaTime;
	}
...
void Draw()
{
	...
	Bg.Draw(pRenderer);
	Player.Draw(pRenderer); // call on the render function to draw on my sprite 
	for (int i = 0; i < bulletcontainer.size(); i++)
	{
		// to acess the elements of an array ,we can use the array acess operators []
		sprite* someprojectiles = &bulletcontainer[i];
